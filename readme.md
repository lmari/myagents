...
* la chiamata a funzioni (_function calling_: https://platform.openai.com/docs/guides/function-calling, in accordo all'endpoint `Chat Completion`: vedi gli esempi in `agenttools.py`, dove aggiungere nuove funzioni); il parametro `tool_handling` consente di specificare se il modello di linguaggio deve operare solo come traduttore dalla richiesta dell'utente a un JSON (`schema`), se il framework deve anche eseguire le funzioni identificate e restituire il risultato (`standard`), o se il framework deve infine chiamare nuovamente il modello e fargli produrre una risposta in linguaggio naturale (`expanded`).
